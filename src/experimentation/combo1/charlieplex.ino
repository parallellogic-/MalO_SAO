...
void Charlieplex::begin(){
    ...
    if(_sm_offset == (uint)-1)
      _sm_offset = pio_add_program(_pio, &charlie_dma_program);//load once in main

    _sm = pio_claim_unused_sm(_pio, true);//for each charlieplex LED (one for lower, one for upper)
    charlie_dma_program_init(_pio, _sm, _sm_offset, _first_pin, 8);
    ...
    for(uint8_t frame_buffer_index=0;frame_buffer_index<CHARLIPLEX_FRAME_BUFFER_DEPTH;frame_buffer_index++)
    {
...
void Charlieplex::flush()
{
    ...
    uint8_t write_index = (_charliplex_index + 1) % CHARLIPLEX_FRAME_BUFFER_DEPTH;
