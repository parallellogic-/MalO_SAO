...
#define CHARLIEPLEX_LED_COUNT (24*2)//24 reg-green LEDs
...
class Charlieplex{
  private:
    uint8_t _api_brightness[CHARLIEPLEX_LED_COUNT];//write brightness values here for the frame currently being developed (red in lower 24 indexes, green in the upper 24)
    uint32_t _charliplex_list[2][CHARLIEPLEX_LED_COUNT+1];//31..16 is brightness, 15..8 is input(0) vs output(1) for 8 pins, 7..0 is high(1) vs low(0) for 8 pins.  +1 for a sleep statement for brightness stabalization
    uint8_t _max_effective_led_count=CHARLIEPLEX_LED_COUNT;//max number of LED elements that are expected to be on simultaneously: drives refresh frame rate
