...
class SharedDecoderBuffer{
  private: 
    uint32_t _data_buffer[SHARED_BUFFER_LENGTH] __attribute__((aligned(SHARED_BUFFER_LENGTH)));
    int _data_dma=-1;
    PIO _pio=pio1;
    int _sm=-1;
  ...
  public:
    SharedDecoderBuffer(PIO pio=pio1,uint8_t rxd_first_pin=SHARED_BUFFER_FIRST_PIN,uint8_t rxd_pin_count=SHARED_BUFFER_PIN_COUNT);
    void begin();//setup DMA ring
    ...
    bool is_activity(int8_t pin_index,uint32_t timeout_us);//true if there has been any 1/0 or 0/1 transitions in the past timeout_us
};
...
class DecoderWS2812{
  private:
    ...
    uint8_t _rxd_pin_index=-1; //index of hte pin wihtin the pio_sm output to inspect
  ...
  public:
    DecoderWS2812(SharedDecoderBuffer* buffer,uint8_t rxd_pin_index,int8_t pwm_activity_pin=-1);
    void begin();
...
class DecoderGeneric{
  private:
    ...
    uint16_t _decode_index=0;//index where next byte should be written to
  public:
    DecoderGeneric(SharedDecoderBuffer* buffer,uint8_t rxd_pin_index);
    void begin();
    void update();
    bool get_message(uint32_t &message, uint16_t &message_length); //returns a series of '1'/'0'
};*/